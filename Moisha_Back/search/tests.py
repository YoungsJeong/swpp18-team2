...
class SearchTestCase(TestCase):
    ...
    def testSearchDepartment(self):
        response = self.client.get('/api/search/department/', {'q': ''})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/department/', {'q': 'test'})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/department/', {'q': 'department'})
        self.assertEqual(response.status_code, 200)

    def testSearchArticleTag(self):
        response = self.client.get('/api/search/articletag/', {'q': ''})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/articletag/', {'q': 'a'})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/articletag/', {'q': 't'})
        self.assertEqual(response.status_code, 200)

    def testSearchInterestTag(self):
        response = self.client.get('/api/search/interesttag/', {'q': ''})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/interesttag/', {'q': 'q'})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/interesttag/', {'q': 't'})
        self.assertEqual(response.status_code, 200)

    def testSearchInterest(self):
        response = self.client.get('/api/search/interest/', {'q': ''})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/interest/', {'q': ' '})
        self.assertEqual(response.status_code, 200)
    ...
    def testSearchInterestByUser(self):
        ...
        response = self.client.get('/api/search/interest/user/', {'q': ''})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/interest/user/', {'q': 'q'})
        self.assertEqual(response.status_code, 200)
        response = self.client.get('/api/search/interest/user/', {'q': 'interest'})
        self.assertEqual(response.status_code, 200)